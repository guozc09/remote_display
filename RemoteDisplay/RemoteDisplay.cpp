 ...
 * @LastEditors  : Zhc Guo
 * @LastEditTime : 2020-01-15 17:01:39
 */
#include <iostream>
...
namespace remote_display
{
    ...
    RemoteDisplaySender::RemoteDisplaySender(TransmissonType type) {
        switch (type) {
            case TYPE_SOCKET_NET:
                mTransmissionClient = new TransmissionClientLo();
                break;
            default:
    ...
    RemoteDisplayReceiver::RemoteDisplayReceiver(TransmissonType type, shared_ptr<RemoteDisplayPlayer> &player) {
        switch (type) {
            case TYPE_SOCKET_NET:
                mTransmissionServer = new TransmissionServerLo(this);
                break;
            default:
                cerr << "failed!! Not currently supported type: " << type << endl;
                break;
